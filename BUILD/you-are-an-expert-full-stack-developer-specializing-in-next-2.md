...
2. **Retainer Invoicing Alerts & Automated Client Emails (Edge Function):**
   ...
   - When triggered, it must execute a Supabase Edge Function that does two things simultaneously:
     1. Automatically draft and send an email to the client's email address detailing the fixed `billing_amount` due for their specific `service_type`.
     2. Send a simultaneous notification alert to me (the agency admin) via webhook (Telegram/WhatsApp placeholder) confirming that the invoice reminder email has been successfully dispatched to the client.
