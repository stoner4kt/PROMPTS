...
I want to make these edits to my system, this is the prompt I will feed into codex,I want a improved prompt and a setup Guide to setup this cloudinary upload feature :[Act as an expert Full-Stack Software Engineer specializing in the Supabase and Cloudinary ecosystems. 
...
I ran the prompt in codex and got this bug review:{**<sub><sub>![P1 Badge](https://img.shields.io/badge/P1-orange?style=flat)</sub></sub>  Keep itinerary_url as text or migrate the column**

Uploading an itinerary now assigns an object to `payload.itinerary_url`, but `schema.sql` still defines `bookings.itinerary_url` as `TEXT` and this change does not include a migration to change that column. In the admin booking form, any submission with an itinerary file will therefore be sent to Supabase/PostgREST with a JSON object for a text field and fail instead of saving the booking/itinerary; either store a string-compatible value or migrate the column and update all readers} . Provide me with a prompt I can feed into codex to fix this issue.
